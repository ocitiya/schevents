  ...
  <div id="lp" class="content">
    ...
    <div class="data-container">
      <div class="data-header">
        <a href="{{ route('admin.lp.sport.create') }}" class="btn btn-primary btn-sm unrounded">
          Tambah LP Sport&nbsp;
...
@section('script')
  <script>
    $('#datatable').on('click', '.delete', function () {
      ...
      const deleteURL = `/admin/lp/sport/delete`;
      const formData = new FormData();
      formData.append('id', id);
      formData.append('_token', csrfToken);

      swal({
        text: `Ingin menghapus LP Sport ${name}?`,
        icon: "warning",
        buttons: true,
        dangerMode: true,
      })
    ...
    document.addEventListener('DOMContentLoaded', async function () {
      $(function () {
        table = $('#datatable').DataTable({
          ...
          serverSide: true,
          ajax: {
          url: "/api/lp/sport/listDatatable"
          },
          columns: [
            ...
            {data: 'id', title: 'Aksi', name: 'action', orderable: false, searchable: false,
              "render": function ( data, type, row, meta ) {
                let updateRoute;
                updateRoute = `/admin/lp/sport/update/${data}`;

                return `
                  <div>
                    <a href="${updateRoute}" class="btn btn-sm unrounded btn-primary">
                      <small>Edit LP Olahraga</small>
                    </a>
                  </div>
